...
function apply(options, book) {
    ...
    book.balance = function(from, to) {
        function getPreviousBalance(book, from) {
            var fromDate = bookie.parseDate(from);

            if(fromDate) {
                fromDate.setDate(fromDate.getDate() - 1);
                return book.balance(null, fromDate);
            }
        }
        ...
        if(from) {
            output.from = _.isString(from) ? from : bookie.dateToString(from);
        }
        ...
        var previousBalance = getPreviousBalance(this, from);
        var ingoingOwnCapital = previousBalance ? previousBalance.ownCapital.outgoing : 0;

        var sumAssets = sumAccounts(this.getAccounts(types.asset), true, from, to, resultVerificationFilter);
        var sumOwnCapital = sumAccounts(this.getAccounts(types.ownCapital), false, from, to, resultVerificationFilter);
        var sumDebts = sumAccounts(this.getAccounts(types.debt), false, from, to, resultVerificationFilter) - sumOwnCapital;
        ...
        var valid = sumAssets === (sumDebts + ownCapital);
        ...
        if(!this.singleOwner()) {
            ...
            _.forEach(this.owners, function(owner) {
                ownCapitalShare[owner.name] = {
                    ...
                    outgoing: bookie.round(sumAccounts(this.getAccounts(types["ownCapitalOwner" + n]), false, from, to, resultVerificationFilter) + result.resultShare[owner.name])
                };
