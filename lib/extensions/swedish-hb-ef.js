...
function apply(options, book) {
    ...
    //TODO: Use incoming own capital for each owner here.
        ...
        if(!this.singleOwner()) {
            ...
            _.forEach(this.owners, function(owner) {
                ownCapitalShare[owner.name] = {
                    ingoing: 0,
                    outgoing: bookie.round(sumAccounts(this.getAccounts(types["ownCapitalOwner" + n]), from, to) + result.resultShare[owner.name])
                };
