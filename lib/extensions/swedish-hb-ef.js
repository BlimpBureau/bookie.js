...
function apply(options, book) {
    ...
    book.balance = function(from, to) {
        ...
        if(from) {
            output.from = from; //TODO: make sure string date.
        }

        if(to) {
            output.to = to; //TODO: make sure string date.
        }
        ...
        if(!this.singleOwner()) {
            ...
            _.forEach(this.owners, function(owner) {
                ownCapitalShare[owner.name] = {
                    ...
                    outgoing: bookie.round(sumAccounts(this.getAccounts(types["ownCapitalOwner" + n]), false, from, to) + result.resultShare[owner.name])
                };
